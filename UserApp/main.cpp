...
void Main(void)
{
    ...
    // Respond(*uart6StreamOutputPtr, "[sys] Heap remain: %d Bytes\n", xPortGetMinimumEverFreeHeapSize());
...
void ThreadUartTx(void* argument)
{
    for (;;)
    {
        ...
        // Suspended here until got Notification.
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        memcpy(joint_angles + 0 * sizeof(float), &dummy.currentJoints.a[0], sizeof(float));
        ...
        joint_angles[24]=0x00;
        joint_angles[25]=0x00;
        joint_angles[26]=0x80;
        joint_angles[27]=0x7f;
        HAL_UART_Transmit_DMA(&huart6, (uint8_t*)joint_angles, 28);
