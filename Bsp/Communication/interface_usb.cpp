...
USBSender usb_packet_output_cdc(CDC_OUT_EP, sem_usb_txHandle);
...
static void UsbServerTask(void *ctx)// 任务函数
{
    ...
    for (;;)
    {
        ...
        osStatus sem_stat = osSemaphoreAcquire(sem_usb_rxHandle, osWaitForever);
        if (sem_stat == osOK)
        {
            usb_stats_.rx_cnt++;
...
void usb_rx_process_packet(uint8_t *buf, uint32_t len)
{
    ...
    osSemaphoreRelease(sem_usb_rxHandle);
}
