...
class UART6Sender : public StreamSink
{
public:
    UART6Sender()
    {
        channelType = CHANNEL_TYPE_UART6;
    }
    ...
    int process_bytes(const uint8_t* buffer, size_t length, size_t* processed_bytes) override
    {
        ...
        while (length)
        {
            ...
            if (osSemaphoreAcquire(sem_uart6_tx_dmaHandle, PROTOCOL_SERVER_TIMEOUT_MS) != osOK)
                return -1;
            // transmit chunk
            memcpy(tx_buf_, buffer, chunk);
            if (HAL_UART_Transmit_DMA(&huart6, tx_buf_, chunk) != HAL_OK)
                return -1;
            buffer += chunk;
            length -= chunk;
            if (processed_bytes)
...
StreamSink* uart6StreamOutputPtr = &uart6_stream_output;
...
static void UartServerTask(void* ctx)
{
    ...
    for (;;)
    {
        ...
        if (huart6.ErrorCode != HAL_UART_ERROR_NONE)
        {
            ...
            HAL_UART_Receive_DMA(&huart6, dma_rx_buffer[0], sizeof(dma_rx_buffer[0]));
        }
        // Fetch the circular buffer "write pointer", where it would write next
        uint32_t new_rcv_idx = UART_RX_BUFFER_SIZE - huart6.hdmarx->Instance->NDTR;
        ...
        if (new_rcv_idx < dma_last_rcv_idx[0])//这里主要是对于DMA循环接收数据的处理，尤其对于重装寄存器更新的时候的处理
        {
            ...
            ASCII_protocol_parse_stream(dma_rx_buffer[0] + dma_last_rcv_idx[0],
                                        UART_RX_BUFFER_SIZE - dma_last_rcv_idx[0], uart6_stream_output);
        ...
        if (new_rcv_idx > dma_last_rcv_idx[0])
        {
            ...
            //                                  nullptr); // TODO: use process_all
            ASCII_protocol_parse_stream(dma_rx_buffer[0] + dma_last_rcv_idx[0],
                                        new_rcv_idx - dma_last_rcv_idx[0], uart6_stream_output);
...
void StartUartServer()//启动dma搬运uart数据
{
    ...
    HAL_UART_Receive_DMA(&huart6, dma_rx_buffer[0], sizeof(dma_rx_buffer[0]));//这里启动DMA的时候会告诉DMA期待接收的数据量
    dma_last_rcv_idx[0] = UART_RX_BUFFER_SIZE - huart6.hdmarx->Instance->NDTR;
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef* huart)//串口发送完成会自动调用这个回调函数释放信号量
{
    if (huart->Instance == USART6)
        osSemaphoreRelease(sem_uart6_tx_dmaHandle);
