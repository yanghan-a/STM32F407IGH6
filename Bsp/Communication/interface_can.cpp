...
//    ctx->tx_msg_cnt++;
    ...
    if (hcan->Instance == CAN1)
        ...
        osSemaphoreRelease(sem_can2_txHandle);
}
...
void CanSendMessage(CAN_context* canCtx, uint8_t* txData, CAN_TxHeaderTypeDef* txHeader)
{
    ...
    if (canCtx->handle->Instance == CAN1)
        semaphore_status = osSemaphoreAcquire(sem_can1_txHandle, osWaitForever);
    else if (canCtx->handle->Instance == CAN2)
        semaphore_status = osSemaphoreAcquire(sem_can2_txHandle, osWaitForever);
    else
        return;
